  ...
  describe "header links" do
    context "with signed in admin user" do
      ...
      it "has invitation link" do
        expect(find(:test_id, "navigation_list")).to
        have_link(I18n.t("layouts.header.invitation"), href: new_user_invitation_path)
      end

      it "has sign out link" do
        expect(find(:test_id, "navigation_list")).to
        have_link(I18n.t("layouts.header.sign_out"), href: destroy_user_session_path)
      end
    end
    ...
    context "with not signed in user" do
      ...
      it "has sign in link" do
        expect(find(:test_id, "navigation_list")).to
        have_link(I18n.t("layouts.header.sign_in"), href: new_user_session_path)
      end
